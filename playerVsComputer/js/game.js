...
$('#difficultyLevel').click(function() { 
	if (movesCounter == 0 ) { //don't let users change difficulty mid-game
		if (difficulty === 'easy') {
			difficulty = 'medium';
...
$('#resetGame').click(function() {
	...
	sweetAlert({
		...
		if (confirmed) {
			...
			userWins = 0;
			resetGameBoard();
			$('#userScore').text('player wins: ' + 0);
			$('#aiScore').text('player wins: ' + 0);
		}
	});
});
...
function whosOnFirst() {
	...
	//console.log("checking turn value:"+determineFirst);
...
function checkWin(playerArr, player) {
	...
	//console.log("move counter"+movesCounter)
	changeTurn();
}
...
function userMove() {
	$('.board div').click( function() {
		...
		console.log("check1: "+ this.id);
		playerMove = (playerMove.split("-").pop()) * 1;
		console.log("check2: "+ playerMove);
...
function checkPotentialWins(playerArr) {
		...
		function checkSubArrays() {
			winningMove = subArray.every(function(val) {
				console.log("val"+val);
				console.log("winning value:"+winningMove)
				return playerArr.indexOf(val) >= 0;
